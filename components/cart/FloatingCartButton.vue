  ...
  <ClientOnly>
    <Teleport to="body">
      ...
      >
        <div 
          v-if="isCartOpen" 
          class="fixed inset-0 bg-black/80 z-[9998]"
          @click="closeCart"
        />
      </Transition>
      ...
      >
        ...
        >
          ...
          <div v-if="loading || isUpdating" class="flex-1 flex items-center justify-center">
            <div class="w-12 h-12 border-2 border-black border-t-accent animate-spin" />
          </div>
          
          <!-- Empty Cart -->
          <div v-else-if="isEmpty" class="flex-1 flex flex-col items-center justify-center px-6">
            <div class="w-24 h-24 border-2 border-black/20 flex items-center justify-center mb-6">
              <span class="font-display text-4xl text-black/20">∅</span>
            </div>
            <p class="font-sans text-black/50 text-center">{{ t.emptyCart }}</p>
          </div>
