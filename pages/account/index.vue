      ...
      <div class="grid lg:grid-cols-4 gap-8">
        ...
        <main class="lg:col-span-3 space-y-6">
          ...
          <div class="bg-neutral-900 rounded-2xl p-8">
            ...
            <p class="text-lg text-neutral-400">{{ t.welcomeSubtitle }}</p>
          </div>
          ...
          <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div class="bg-neutral-900 rounded-2xl p-5">
              ...
              <p class="text-sm text-neutral-500">{{ t.totalOrders }}</p>
            </div>
            ...
            <div class="bg-neutral-900 rounded-2xl p-5">
              ...
              <p class="text-sm text-neutral-500">{{ t.totalSpent }}</p>
            </div>
            ...
            <div class="bg-neutral-900 rounded-2xl p-5">
              ...
              <p class="text-sm text-neutral-500">{{ t.totalItems }}</p>
            </div>
            ...
            <div class="bg-neutral-900 rounded-2xl p-5">
              ...
              <p class="text-sm text-neutral-500">{{ t.savedAddresses }}</p>
            </div>
          </div>
          ...
          <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <NuxtLink
              to="/account/orders"
              class="bg-neutral-900 rounded-2xl p-6 hover:bg-neutral-800 transition-colors group"
            >
              <div class="w-14 h-14 rounded-xl bg-neutral-800 group-hover:bg-neutral-700 flex items-center justify-center mb-4 transition-colors">
                <svg class="w-7 h-7 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
                </svg>
              </div>
              <h3 class="text-xl font-semibold text-white mb-2">{{ t.orders }}</h3>
              <p class="text-neutral-400">{{ t.ordersDescription }}</p>
            </NuxtLink>
            ...
            >
              ...
              <h3 class="text-xl font-semibold text-white mb-2">{{ t.addresses }}</h3>
              <p class="text-neutral-400">{{ t.addressesDescription }}</p>
            </NuxtLink>
            ...
            >
              ...
              <h3 class="text-xl font-semibold text-white mb-2">{{ t.continueShopping }}</h3>
              <p class="text-neutral-400">{{ t.continueShoppingDescription }}</p>
            </NuxtLink>
          </div>
          
          <!-- Customer Info -->
          <div class="bg-neutral-900 rounded-2xl p-8">
            <h2 class="text-xl font-semibold text-white mb-6">{{ t.accountInfo }}</h2>
            
            <div class="grid sm:grid-cols-2 gap-6">
              <!-- Email -->
              <div>
                <p class="text-sm text-neutral-500 mb-1">{{ t.email }}</p>
                <p class="text-lg text-white">{{ customer?.email || '-' }}</p>
              </div>
              
              <!-- Name -->
              <div>
                <p class="text-sm text-neutral-500 mb-1">{{ t.name }}</p>
                <p class="text-lg text-white">{{ displayName || '-' }}</p>
              </div>
              
              <!-- Billing Address -->
              <div v-if="billingAddress">
                <p class="text-sm text-neutral-500 mb-1">{{ t.billingAddress }}</p>
              ...
              <div v-if="shippingAddress">
                <p class="text-sm text-neutral-500 mb-1">{{ t.shippingAddress }}</p>
                <p class="text-lg text-white">
          ...
          <div class="bg-neutral-900 rounded-2xl p-8">
            <div class="flex items-center justify-between mb-6">
              <h2 class="text-xl font-semibold text-white">{{ t.recentOrders }}</h2>
              <NuxtLink 
                to="/account/orders" 
            ...
            <div v-else-if="recentOrders.length > 0" class="space-y-4">
              <NuxtLink
                ...
                class="flex items-center justify-between p-5 bg-neutral-800 rounded-xl hover:bg-neutral-700 transition-colors"
              >
                <div>
                  <p class="text-lg font-medium text-white">{{ t.orderNumber }} {{ order.orderNumber }}</p>
                  <p class="text-neutral-400">{{ formatDate(order.orderDateTime) }}</p>
                </div>
                <div class="text-right">
                  <p class="text-lg font-semibold text-white">{{ formatPrice(order.amountTotal, order.currency) }}</p>
            ...
            <div v-else class="text-center py-12">
              ...
              <p class="text-lg text-neutral-400 mb-4">{{ t.noOrders }}</p>
              <NuxtLink 
                to="/" 
                class="inline-block px-6 py-3 bg-white text-neutral-900 rounded-xl font-semibold hover:bg-neutral-100 transition-colors"
              >
                {{ t.startShopping }}
              </NuxtLink>
            </div>
